...
tags:
  - skimr
subtitle: ''
summary: 'Develpoed by the rOpenSci conmmunity, skimr is designed to provide useful, tidy summary statistics about variables.'
bibliography: bib/introduction-to-skimr.bib
biblio-style: "apalike"
link-citations: true
lastmod: '2019-11-22T23:27:44+08:00'
featured: no
image:
  caption: ''
  focal_point: ''
...
1.[`skim()` basics](#intro)  
2.[Manipulating the results](#manipulation)  
3.[Customizing `skim()`](#customization)   
4.[Skimming non-data frames](#other)  
5.[References](#ref)
...
The **skimr**[@R-skimr] package is developed by the [rOpenSci](https://github.com/ropensci) cummunity, with its core function `skim()` as an enhanced version of `summary()`, and has just experienced a major update to V2. ^[In the sense that `skim()` provides useful, tidy summary statistics and display it in a pretty form for exploratory analysis based on data frames. `skim()` cannot supplant `summary()` in terms of model interpretation, viewing geospatial objects, etc.] 
...
As noted above, `skim()` returns a wide data frame, an `skim_df` object. This is usually the most sensible format for the majority of operations when investigating data, so the package provides certain functions to convert `skim_df` into more useful data types(i.e., data frames).  
...
This is useful when we only want to look at certain data types, we can subset that component out of the `partition` result, coerce it to a tibble, and then manipulate it. We don't have to put a prefix before column names now, since rows in every component are of identical data types, and therefore share same statistics:  
...
when skimming matrices, columns and treated as variables and rows and records, so statistics calculated are in fact column-wise:  
