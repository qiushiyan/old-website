...
This week I've been doing some recap on how to do basic data processing and cleaning in Python with the `pandas` and `NumPy` library. So this post is mostly a self-remainder on how to deal with messy data in Python, by reproducing data cleaning examples presented in Hadley Wickham's [Tidy Data](https://vita.had.co.nz/papers/tidy-data.pdf) paper, @JSSv059i10. 
...
Messy data are, by extension, datasets in volation againest these 3 rules. The author then described the five most common problems with messy datasets:  
...
In this post I will be focusing on  the first 3 symptoms since the other two violations often occur when working with databases. All [datasets]((https://github.com/chendaniely/scipy-2019-pandas/tree/master/data)) are from Hadley's paper [repo](https://github.com/hadley/tidy-data) and [Daniel Chen](https://chendaniely.github.io/)'s 2019 SciPy talk on data manipulation with pandas.
...
Thanks to the [`reticulate`](https://rstudio.github.io/reticulate/), we can mix R and Python code seamlessly. Here is a line plot made with `seaborn` demonstrating total deaths per hour:
...
Here we see causes of death with highest `dist` are mainly accidents and rare diseases. However, there is a negative correlation between the frequency of a cause and its deviation, as shown in the following plot.
...
Thus, our final solution is to build a model with `n` as predictor, andn `dist` as response. And cause with highest residual are assumed to have the most deviation. Since the linear trend fits the data quite well, I'm simply going to use linear regression (Hadley use robust linear model in the paper). 
