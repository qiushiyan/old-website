...
To obtain tidy text structure illustrated in [Text Mining with R](https://www.tidytextmining.com/), I use `unnest_tokens()` to perform tokenization and remove all the stop words. I also removed characters like `'`, `'s`, `'` and whitespaces to return valid column names after widening. But it turns out this served as some sort of stemming too! (heathcliff's becomes heathcliff). Then low frequency words (whose frequency is less than 0.05% of an author's total word counts) are removed. The cutoff may be a little too high and if you plot the histogram of that proportion, but I really need this to save computation efforts on my laptop :sweat_smile:. 
...
Before building an actual predictive model, lets' do some EDA to see different usage tendency on the same word! This will also shed light on what we would expect from the text classification. Now, we will compare word frequency (proportion) between the two sisters.
...
Words lie near the line such as "home", "head" and "half" indicate similar tedency to use that word, while those that are far from the line are words that are found more in one set of texts than another, for example "headthcliff", "linton", "catherine", etc. 
...
There are `r nrow(dplyr::count(tidy_books, word))` and features (words) and `r nrow(dplyr::count(tidy_books, line_index))` observations in total. Approximately 18% fo the response are 1 (Emily Brontë).
...
Here and I build a set of 10 cross validations resamples, and set `levels = 100` to try 100 choices of lambda ranging from 0 to 1.  
...
Ok, the two metrics both display a monotone decrease as lambda increases, but does not exhibit much change once lambda is greater than 0.1, which is essentailly random guess. 
```{r}
best_lambda <- logistic_results %>% 
  select_best(metric = "roc_auc")

book_wf_final <- finalize_workflow(logistic_wf_tune,
                                  parameters = best_lambda)
```

Now the model specification in the workflow is filled by the best lambda ($\lambda = 0.0007$)at hand judged by cross validation. 
```{r}
...
The accuracy of our logisitc model rises by a rough 7% to 93%, with `roc_auc` being nearly 91.5%. This is pretty good!

To evaluate model fit, I agian use `fit` and `pull_workflow`. Variable importance plots implemented in the [vip](https://koalaverse.github.io/vip/index.html) provides a intuitive way to visualize importance of predictors in this scenario, using th absolute value of the t-statistic as a measure of VI.


```{r, fig.height = 11, fig.width = 11, fig.cap = "Variable importance for penalized logistic regression"}
library(vip)
...
Note that variale importance in the left panel is generally smaller than the right, this corresponds to what we find in the word frequency plot that Emily Brontë has more and stronger characteristic words. I am not sure if this is partly because imbalance in our data? 

Anyway, the model with the best lambda seems quite power in distinguishing these two authors. I look forward to build a multinomail classification model in `tidymodels` to include Anne Brontë some other time!
