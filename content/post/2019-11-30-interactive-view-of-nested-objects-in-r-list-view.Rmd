...
author: Qiushi Yan
date: '2019-11-30'
slug: interactive-view-of-nested-objects-in-r-list-viewer
categories:
  - R
tags:
  - JSON
...
featured: no
bibliography: bib/listviewer.bib
...
A problem is that, this is by far where we could go with static output, we cannot tell `str()` to go deeper into one component while others stay in their level , or get a more visually appearing display of the object's structure. We  are either overwhelmed or constained.  

This is when a taste of interactivity might help. The **listviewr** package[@R-listviewer] contains R htmlwidgets to interactively view and maybe modify nested and complicated objects. In fact, listviewer provides an interface to 2 interactive JSON editors: [react-json-view](https://github.com/mac-s-g/react-json-view) and [jsoneditor](https://github.com/josdejong/jsoneditor). They are well designed to interactively visualize JSON data, which is also commonly hierarchical and sophisticated. JSON data in JavaScript, in its nature, are equivalent to R's list object. For this reason, any object that have a list-like structure (e.g, data frames, named vector, and of course lists) in R could be visualized in **listviewer**.    
