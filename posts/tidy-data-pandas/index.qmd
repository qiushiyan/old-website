...
If we are to answer questions like "what are the average ranking of artisits across all weeks?", the `wk*` columns need to be transformed into values:  ^[Although both pandas and dplyr (> 1.0) supports rowwise operatios, we will suffer some performance penalties for not utilizing vectorization.]
