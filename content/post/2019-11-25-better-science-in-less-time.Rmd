...
```{r transition,echo=F,fig.cap="Better science in less time, illustrated by the Ocean Health Index project"}
knitr::include_graphics("/img/ohi_transition.png")
```

Figure \@ref(fig:transition) shows an interesting transition of the OHI project from traditional methods to modern data science tools, and most importantly, with greater reproducibility and collaboration effectiveness. As they described:  
...
```{r,echo=F,fig.cap="ohicore: an toolbox in R presented by the project"}
knitr::include_graphics("/img/ohi.png")
```
